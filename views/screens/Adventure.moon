...
class Adventure extends require "orca.core.widget"
	...
	content: =>
		...
		unless config
			...
			return AdventureEmptyState!\render!

		@content_for "title", config.title
		@content_for "header", AdventureHeader(config.title, font, -> navigate "/")\render!
		...
		command_bar = AdventureCommandBar font, (text) ->
			transcript\append session\submit text

		@content_for "footer", command_bar\render!
		command_bar\focus!
		transcript\render!

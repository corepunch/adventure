...
class SignIn extends ui.Form
	...
	body: =>
		...
		p ".title", "Log in with existing account"
		p ".description", "To use mobile banking now"
		for item in *fields 
			ui.Label ".label", For: item.attribute, item.label
			ui.Input "##{item.attribute}.input"
				Name: item.attribute
				PlaceholderText: item.label
		ui.Button ".primary", Type: "Submit", "Sign in"
		ui.Button ".link", Click: (=> routing.navigate "/sign-up"), "No account? Create one now"
		stack ".links", ->
			links = {
			...
			for link in *links
				ui.Button ".small-link", Click: link.func, link.label
